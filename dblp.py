...
from MSHGT.model.DBLP.mshgt import MSHGTModel
...
def load_args():
    ...
    parser.add_argument('--dataset', type=str, default='DBLP', help='name of dataset')  # adjust
    ...
    parser.add_argument('--convergence_epoch', type=int, default=300, help="number of epochs for warmup")  # adjust
...
def main():
    ...
    train_mask, val_mask, test_mask = train_val_test_split(num_nodes=num_nodes, y=data[node_types[0]]['y'], train_p=0.5,
                                                           val_p=0.25)
    # visualize_embedding(data[node_types[0]]['x'][test_mask], data[node_types[0]]['y'][test_mask], name='org_DBLP')
    ...
    model = MFHGTModel(num_nodes=num_nodes, in_dim_a=in_dim_a, in_dim_p=in_dim_p, in_dim_t=in_dim_t,
                       hidden_dim=args.hidden_dim, out_dim=num_classes, num_edge_types=num_edge_types,
                       heads=args.heads, num_layers_gnn=2, num_layers_gt=args.num_layers,
    ...
    # visualize_embedding(outputs=out[test_mask], labels=y[test_mask], name='DBLP')
