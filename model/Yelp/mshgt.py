...
from MSHGT.model.Yelp.GTLayer import GraphTransformerLayer
from MSHGT.model.Yelp.mf import get_all_pe
...
class MSHGTModel(nn.Module):
    def __init__(self, num_nodes, homo_edge_index, hidden_dim, num_edge_types, heads,
        ...
        self.get_embs = Node2Vec(edge_index=homo_edge_index, embedding_dim=hidden_dim, walk_length=16,
                                 context_size=4, walks_per_node=4, p=1, q=1, num_negative_samples=1)
    ...
    def forward(self, data, original_A, pos_index, neg_index, deg):
        ...
        x_dict = {}
        x_dict['BUSINESS'] = embs[0:7474, :]
        x_dict['LOCATION'] = embs[7474:(7474 + 39), :]
        x_dict['STARS'] = embs[(7474 + 39):(7474 + 39 + 9), :]
        x_dict['PHRASE'] = embs[(7474 + 39 + 9):, :]

        edge_index_dict = data.edge_index_dict
        x = self.hgnn(x_dict, edge_index_dict)
        loss_pe, pe_Q, pe_K = self.get_pe(original_A)
        x_gt = self.net(x['BUSINESS'], pe_Q, pe_K, deg)
        x_gt = F.dropout(x_gt, p=self.dropout)
        out = self.mlp(x_gt)

        pred_adj = torch.sigmoid(self.pred_dot(out) @ x['PHRASE'].t() / self.hidden_dim)  # B-P
