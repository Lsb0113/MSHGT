...
class SVD(nn.Module):
    ...
    def forward(self, A):
        ...
        regular_U = 0.5 * torch.norm(torch.matmul(self.U.t(), self.U) - torch.eye(n=self.hidden_dim,device=A.device),
                                     p=2)
        regular_V = 0.5 * torch.norm(torch.matmul(self.V, self.V.t()) - torch.eye(n=self.hidden_dim,device=A.device),
                                     p=2)
        loss = nn.MSELoss()(A, new_A) + regular_U + regular_V

        U = torch.matmul(self.U, torch.sqrt(torch.diag(self.S)))
        V = torch.matmul(torch.sqrt(torch.diag(self.S)), self.V)
        pe_Q = U
        pe_K = V.t()
...
class get_all_pe(nn.Module):
    def __init__(self, num_nodes, hidden_dim, num_edge_types):
        ...
        for i in range(num_edge_types):  # apa aptpa apcpa
