...
class SVD(nn.Module):
    ...
    def forward(self, A):
        ...
        regular_U = 0.5 * torch.norm(self.U.t() @ self.U - torch.eye(n=self.hidden_dim,device=A.device), p=2)
        regular_V = 0.5 * torch.norm(self.V @ self.V.t() - torch.eye(n=self.hidden_dim,device=A.device), p=2)
